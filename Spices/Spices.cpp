...
#define MAX_LINE_WIDTH	100							//CHANGE: maximum line width
#define FILTER_ENDS 	6							//CHANGE: # of pixels at end of camera data to ignore; set to 0 for now, later make 15
...
// steer/servo params
#define MAX_STEER_LEFT  0.07						// CHANGE: value determined by demo mode 1 measure (have to be adjusted with every servo horn attach)
#define MAX_STEER_RIGHT -0.74						// CHANGE: value determined by demo mode 1 measure
...
#define CONTROL_METHOD 	0							// CHANGE: which control method to use

// Drive/motor params
// 0.4 way too slow!!	need to charge battery
#define SPEED_CASE 2
#define SUB_LIGHT_SPEED 0.5							// 2:moderate speed (value 0 to 1 sent to motors)
#define LIGHT_SPEED 	0.6								// 3:fast...
#define RIDICULOUS_SPEED 0.7						// 4:faster...
#define LUDICROUS_SPEED 0.9							// 5:faster still!
...
float		DerivThreshold = 1200;					// CHANGE: Derivative Threshold (default)
...
float      CurrentSteerSetting = ((float)(MAX_STEER_LEFT + MAX_STEER_RIGHT) / 2);  // drive straight at first
...
void MasterControlProgram()
{
	...
	if(TFC_Ticker[0]>LOOP_TIME && TFC_LineScanImageReady>0)
	{
		...
		ActOnTrackStatus(); //SteeringControl() && Steer()
		feedbackLights();
...
void adjustLights(float* InvLineScanDataIn, float* DerivLineScanDataOut)
{
	...
	else if(pairCnt == 1)
	{
		...
		else if(NegEdges[0] > targetPosition)
		{
...
void findEdges(float* derivLineScanData)
{
  ...
  int PosEdgeBufCnt = 0, PosEdgeBufSum = 0;     // serves as buffer to store pos edges found next to each other
	...
	float currentNegEdges[NUM_LINE_SCAN];
	float currentPosEdges[NUM_LINE_SCAN];
	float prevNegEdges[NUM_LINE_SCAN];
	float prevPosEdges[NUM_LINE_SCAN];
  ...
  for(i=minCnt;i<maxCnt;i++) // print one line worth of data from Camera 0
  {      
	 if ((PosEdgeBufSum == 0) && (derivLineScanData[i] == 1 )) 
	 {  // POSITIVE EDGE FOUND!

       PosEdgeBufCnt++;                                      // add value to pos edge buffer
	     numPosEdges++;
		 
		 currentNegEdges[numNegEdges - 1] = numNegEdges;
		 currentPosEdges[numPosEdges - 2] = numPosEdges;
		 prevNegEdges[numNegEdges - 1] = numNegEdges - 1;
		 prevPosEdges[numPosEdges - 2] = numPosEdges - 2;
		 
			 if((numNegEdges - 1 >0) && (prevNegEdges[numNegEdges - 1] - currentNegEdges[numNegEdges - 1]< 30 ))
			{
				currentPosEdges[numPosEdges - 2] = prevPosEdges[numPosEdges - 2];
			}
			else
			{
       PosEdges[numPosEdges - 1] = i;
			}
       PosEdgeBufSum = 1;
       
     }  
     else if ((PosEdgeBufSum == 1) && (derivLineScanData[i] == 0))          // NEGATIVE EDGE FOUND!
     {  
       NegEdgeBufCnt++;                                      // add value to neg edge buffer
       numNegEdges++;
			 
			 if((numNegEdges - 1 >0) && (currentPosEdges[numPosEdges - 2] - prevPosEdges[numPosEdges - 2] < 30))
			{
				currentNegEdges[numNegEdges - 1] = prevNegEdges[numNegEdges - 1];
			}
			else
			{
				NegEdges[numNegEdges - 1] = i;
			}
       PosEdgeBufSum = 0;       
     }  
     else if ((i == maxCnt-1) && (PosEdgeBufSum == 1) && (derivLineScanData[i] == 1))          // NEGATIVE EDGE FOUND!
     {  
       NegEdgeBufCnt++;                                      // add value to neg edge buffer
       numNegEdges++;
       NegEdges[numNegEdges - 1] = maxCnt-1;
       PosEdgeBufSum = 0;       
     }  
  }
}
...
void reviewEdges()
{
  LastTrackStatus = CurrentTrackStatus;
	...
	//TERMINAL_PRINTF("\LINE_WIDTH = %.0f \r\n", NegEdges[0] - PosEdges[0]);
  // only one negative and positive edge found (LINE)
  if ((numPosEdges == 1) && (numNegEdges == 1))  
  {
	// has proper expected width
    if (((NegEdges[0] - PosEdges[0]) >= MIN_LINE_WIDTH) && ((NegEdges[0] - PosEdges[0]) <= MAX_LINE_WIDTH)) 
    {
  ...
  else if ((numNegEdges == 1) && (numPosEdges == 0))  
  {
    ...
    if ((NegEdges[0] >= (MAX_LINE_SCAN - MAX_LINE_WIDTH)) && (LastLinePosError > 0))    
    {
       CurrentTrackStatus = LineFound;                                   // report line found!
       UnknownCount = 0;                                                 // reset unknown status count
       LastLinePosition = CurrentLinePosition;
  ...
  {     
	...
	// more than 1 negative edge and positive edge found (but not 2 for both) (STARTING / FINISH GATE)  
...
void SteeringControl()
{
  ...
  CurrentLinePosError = targetPosition - CurrentLinePosition; // based on pixel location 0-127
	//if(CurrentLinePosError > ) { return; }
  /* Get absolute error
  ...
  switch (CONTROL_METHOD) {
    case 0:
      ...
      KP = (float) ( MAX_STEER_LEFT - MAX_STEER_RIGHT ) / ( NUM_LINE_SCAN - (2*FILTER_ENDS) - MIN_LINE_WIDTH );
      KD = 0;
      KI = 0;
      break;
   case 1:
  ...
  */
  ...
  if (terminalOutput == 1) {
    ...
    // Finally add offset to steering to account for non-centered servo mounting
     ...
     //TERMINAL_PRINTF("%3.3f %3.3f %3.3f %3.3f %3.3f \r\n",CurrentSteerSetting, Pout,Iout,Dout,(float) (MAX_STEER_LEFT + MAX_STEER_RIGHT) / 2 );
    CurrentSteerSetting = Pout + ( (float) (MAX_STEER_LEFT + MAX_STEER_RIGHT) / 2 );
	//TERMINAL_PRINTF("%3.3f %3.3f %3.3f \r\n",CurrentSteerSetting, Pout,(float) (MAX_STEER_LEFT + MAX_STEER_RIGHT) / 2 );
...
void Steer()
{
    ...
    if (terminalOutput == 1) {
      TERMINAL_PRINTF("* CURRENT POSITION %3.3f, ", CurrentLinePosition);
